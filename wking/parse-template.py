...
"""Parse template files replacing <scscript> elements.
"""
...
class TemplateEnvironment (object):
    def __init__(self, prefix='', datadir='.', encoding='utf-8'):
        self.prefix = prefix
...
def parse_template(template, datadir='.', prefix='sc-'):
    """Takes template path and path to data directory
    """
    environment = TemplateEnvironment(prefix=prefix)
    try:
